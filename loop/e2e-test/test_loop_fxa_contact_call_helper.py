...
class TestLoopFxaContactCallHelper(MarionetteTestCase):
    ...
    def local_fxa_enter_password(self):
        ...
        input_box.send_keys(HELPER_PASSWORD)
        time.sleep(3)
    ...
    def local_fxa_contacts_add(self):
        ...
        helper_name.send_keys(MASTER_NAME)
        time.sleep(5)
    ...
    def local_fxa_answer_call(self):
        time.sleep(5)
        button = self.marionette.find_element(
            By.XPATH, 
            "//*[contains(@class,'fx-embedded-answer-btn-text')]" # or contains(@class, 'icon-video')]"
        )
        self.wait_for_element_enabled(button, 3000)
        button.click()
    ...
    def test_loop_fxa_answer_call(self):
